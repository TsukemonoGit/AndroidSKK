...
class SKKDictManager : AppCompatActivity() {
    ...
    private val addDictFileLauncher =
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val dictList =
                skkPrefs.dictOrder // インストール済み辞書をまず列挙
                        .split("/")
                        ...
                        .toMutableList()
        // インストール済みかどうかチェック
        fileList().filter { it.startsWith("skk_dict_") && it.endsWith(".db") }.forEach {
            val entry = it.dropLast(".db".length)
            val type = entry.drop("skk_dict_".length)
            val dupIndex = dictList.indexOfFirst { dict -> entry == dict.value.removePrefix("/") }
            if (dupIndex == -1) {
        ...
        ItemTouchHelper(callback).attachToRecyclerView(binding.dictManagerList)
    }
        ...
        if (newList[position].value.startsWith("/skk_dict_")) {
            ...
            val dialog =
                    ConfirmationDialogFragment.newInstance(
                            getString(R.string.message_dict_manager_confirm_remove_dict)
                    )
            dialog.setListener(
    ...
    override fun onPause() {
        ...
        mDictList.filter { !it.value.startsWith('/') }.forEach {
            dictListString.append(it.key, "/", it.value, "/")
        }
        ...
        if (skkPrefs.dictOrder != dictListString.toString()) {
            ...
            if (SKKService.isRunning()) { // まだ起動していないなら不要
                val intent =
                        Intent(this@SKKDictManager, SKKService::class.java)
                            ...
                            override fun onPositiveClick() {
                                fileList().forEach { file ->
                                    if (!file.startsWith(getString(R.string.dict_name_user)) &&
                                                    !file.startsWith(
                                                            getString(R.string.dict_name_ascii)
                                                    )
                                    ) {
                                        deleteFile(file)
                                    }
    ...
    private fun downloadDict(type: String, position: Int) {
        val dialog =
                ConfirmationDialogFragment.newInstance(
                        getString(R.string.message_dict_manager_confirm_download_dict, type)
                )
        dialog.setListener(
                object : ConfirmationDialogFragment.Listener {
    ...
    private fun loadDict(uri: Uri, position: Int, common: Boolean) {
        ...
        if (name == null) {
            SimpleMessageDialogFragment.newInstance(getString(R.string.error_open_dict))
                    .show(supportFragmentManager, "dialog")
            return
        }
        ...
        val dictFileBaseName =
                if (common && name.startsWith("SKK-JISYO.")) {
        ...
        if (position == -1) { // view がないと進捗が分からないので addDict してからやり直し
            addDict("/$dictFileBaseName", -1) { loadDict(uri, it, false) }
            return
        }
        ...
        MainScope().launch(Dispatchers.IO) {
            ...
            try {
                recMan =
                        RecordManagerFactory.createRecordManager(
                ...
                val charset =
                        if (contentResolver.openInputStream(uri)!!.use { inputStream ->
                                    val processedInputStream =
                                            if (isGzip) GZIPInputStream(inputStream)
                                            else inputStream
            ...
            withContext(Dispatchers.Main) { addDict(dictFileBaseName.removePrefix("/"), position) }
        }
    }
    ...
    class TupleAdapter(private val onItemClickListener: (Int) -> Unit) :
            ListAdapter<Tuple<String, String>, TupleAdapter.TupleViewHolder>(TupleDiffCallback()) {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TupleViewHolder {
            val binding =
                    ActivityCheckedTextBinding.inflate(
    ...
    class TupleItemTouchHelperCallback(private val onMoveListener: (Int, Int) -> Unit) :
